...
temp_cif_path = os.path.join(tempfile.gettempdir(), f"temp-{uuid.uuid4().hex}.cif")
...
def convert_to_p1(ops, asym_unit_atoms, asym_unit_coords):
    ...
    # Pymatgen renamed this parser in newer versions, so support both.
    if hasattr(SymmOp, "from_xyz_str"):
        parse_sym_op = SymmOp.from_xyz_str
    elif hasattr(SymmOp, "from_xyz_string"):
        parse_sym_op = SymmOp.from_xyz_string
    else:
        raise AttributeError(
            "SymmOp has neither from_xyz_str nor from_xyz_string in this pymatgen version"
        )

    for op in ops:
        op_matrices.append(parse_sym_op(op).affine_matrix)
...
def csd_to_pymatgen(path, atoms):
        ...
        if "loop_" in line:
            for block in blocks.keys():
                ...
                if test_header.startswith(block):
                    if (
                        block == "atom_site"
                        and test_header.startswith("atom_site_aniso")
                    ):
                        pass
                    else:
    ...
    struct = Structure(lattice=lattice, species=species, coords=coords, to_unit_cell=True)
...
def main(write_path, read_path=None, tmp_path=None, input_is_cif=False):
    ...
    if input_is_cif:
        ...
        ref = os.path.basename(write_path).rsplit("_P1.cif", 1)[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--read_dir", type=str, default=os.getcwd(), help="Directory of where the cif is located"
    )
    parser.add_argument(
        "-d", action="store_true", help="If this flag is present, " + " keep the temporary CSD cif."
    )
    parser.add_argument(
        "-inp_is_cif",
        action="store_true",
    ...
    if inp_is_cif:
        ...
        read_path = os.path.join(args.read_dir, refcode)
    final_cif_path = os.path.join(write_dir, f"{refcode}_P1.cif")
    if args.d:
        main(
            final_cif_path,
            tmp_path=os.path.join(write_dir, f"{refcode}_original.cif"),
